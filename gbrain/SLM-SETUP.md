# Running GBrain on a local small model

Setup and design notes for the vendored, SLM-retuned GBrain in this directory.
For the pinned upstream commit and the full inventory of local changes, see
...
[GBrain](https://github.com/garrytan/gbrain) in this directory — a Postgres + pgvector
brain with hybrid retrieval, a zero-LLM knowledge graph, cited synthesis with gap analysis, and
a crash-safe job queue. See [`UPSTREAM.md`](UPSTREAM.md) for the pinned upstream
